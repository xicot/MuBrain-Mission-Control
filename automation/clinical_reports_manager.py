...
def upload_pdf_to_drive(service, pdf_path, folder_id):
    ...
    try:
        ...
        # Explicitly close media before cleanup
        if hasattr(media, '_fd') and media._fd:
            media._fd.close()
...
def save_and_upload_report(report_type='weekly'):
    """Generate report, convert to PDF, and upload to Google Drive"""
    print(f"🔄 Generating {report_type} clinical report...")
    
    # Generate report
    if report_type == 'weekly':
        html_content, metadata = generate_weekly_report()
        filename_base = f"clinical-weekly-{datetime.now().strftime('%Y-%m-%d')}"
    else:
        print("Monthly report - using weekly structure as base")
        html_content, metadata = generate_weekly_report()
        filename_base = f"clinical-monthly-{datetime.now().strftime('%Y-%m-%d')}"
    
    # Save HTML locally (temp)
    ...
    if convert_html_to_pdf(html_path, pdf_path):
        ...
        print("⚠️ PDF conversion failed, keeping HTML")
    ...
    if drive_service and REPORTS_FOLDER_ID:
        ...
        upload_result = upload_pdf_to_drive(drive_service, pdf_path, REPORTS_FOLDER_ID)
